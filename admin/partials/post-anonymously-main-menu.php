<?php
/**
 * Post_Anonymously_Main_Menu Main Menu Class.
 *
 * @since Post_Anonymously_Main_Menu 1.0.0
 */
 ...
 *
 * @since      1.0.0
 * @package    Post_Anonymously_Main_Menu
 * @subpackage Post_Anonymously_Main_Menu/includes
 */
class Post_Anonymously_Main_Menu {
	...
	public function main_menu() {
		add_menu_page(
			__( 'Post Anonymously', 'post-anonymously' ),
			__( 'Post Anonymously', 'post-anonymously' ),
			'manage_options',
			'post-anonymously',
			array( $this, 'about' )
	...
	public function about() {
		...
		<style>
			...
			.post-anonymously-logo img {
				max-width: 200px;
				height: auto;
			}
	
			.post-anonymously-content {
				text-align: center;
				max-width: 600px;
				margin-top: 20px;
				padding: 20px;
		...
		<div class="post-anonymously-container">
	
			<div class="post-anonymously-content">
	...
	public function plugin_action_links( $links, $file ) {
		...
		if ( POST_ANONYMOUSLY_PLUGIN_BASENAME !== $file ) {
			return $links;
		}
		...
		return array_merge(
			$links,
			array(
				'about'	=> sprintf( '<a href="%sadmin.php?page=%s">%s</a>', admin_url(), 'post-anonymously', esc_html__( 'About', 'post-anonymously' ) ),
			)
		);
	}
}